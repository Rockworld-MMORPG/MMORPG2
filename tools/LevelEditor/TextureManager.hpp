...
public:
	TextureManager()
	{
		auto success = m_textureAtlas.create(sf::Vector2u(TILE_SCALE * ATLAS_SIZE_X, TILE_SCALE * ATLAS_SIZE_Y));
		if (!success)
		{
			spdlog::error("Failed to create texture atlas");
		}
	}
	...
	auto addTexture(const std::uint32_t identifier, const sf::Image& image) -> void
	{
		...
		auto fCoordX = static_cast<float>(iCoordX);
		auto fCoordY = static_cast<float>(iCoordY);
		auto fWidth  = static_cast<float>(iWidth);
		auto fHeight = static_cast<float>(iHeight);
