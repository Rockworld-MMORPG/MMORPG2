...
TerrainTile::TerrainTile(const Common::World::Level& level, const TextureManager& textureManager)
{
	...
	for (auto yPos = std::size_t(0); yPos < Common::World::LEVEL_HEIGHT; ++yPos)
	{
		for (auto xPos = std::size_t(0); xPos < Common::World::LEVEL_WIDTH; ++xPos)
		{
			...
			for (auto i = std::size_t(0); i < VERTICES_PER_TILE; ++i)
			{
				...
				m_vertexArray[index + i].texCoords = sf::Vector2f(0.0F, 0.0F);
			}

			auto textureCoordinates = textureManager.getTextureCoords(level.getTile(xPos, yPos));
